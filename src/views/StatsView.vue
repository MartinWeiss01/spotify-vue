...
<template>
  ...
  <div class="flex-column">
    <DateToggleButton :time_ranges="time_ranges" @get-time-range-index="changeTimeRange" class="mt-10"></DateToggleButton>  
    <!-- TODO fix Toggle Button  -->
    <div class="mt-10">
      <YourTopArtists v-if="topArtists.length !== 0" :artists="userStore.topArtists.items" />
      <MediaItemSkeleton title="Your Top Artists" v-else />
    </div>
    
    <div class="mt-10">
      <YourTopTracks v-if="topTracks.length !== 0" :tracks="userStore.topTracks.items" />
